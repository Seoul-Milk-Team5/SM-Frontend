    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[653px] h-[800px] p-[50px] rounded-[10px] max-h-[100vh] overflow-y-auto">
        ...
        <div className="mt-6 space-y-4 text-gray-800 text-body-lg">
          <div className="flex justify-between"><span className="text-gray-300">승인번호</span><span>{invoiceInfo?.result.issueId}</span></div>
          <div className="flex justify-between mb-3"><span className="text-gray-300">작성일자</span><span>{invoiceInfo?.result.erDat}</span></div>
          
          <div className="flex justify-between border-t border-[#EEEFEF] pt-3"><span className="text-gray-300">공급자명</span><span>{invoiceInfo?.result.suName}</span></div>
          <div className="flex justify-between"><span className="text-gray-300">공급자 사업자등록번호</span><span>{invoiceInfo?.result.suId}</span></div>
